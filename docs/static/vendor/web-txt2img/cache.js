// Bump this together with an incompatible vendored SDXS model/adapter revision.
// The browser model URL also carries the revision, so immutable HTTP responses
// cannot accidentally reuse an older ONNX binary after a deployment.
const CACHE_NAME = 'web-txt2img-cvclass-sdxs-20260711-browser-clip1';
// Simple in-memory URL tracker per model for purge; re-created on reload.
const urlTracker = new Map();

async function throwFetchError(response, url) {
    let message = '';
    try {
    ...
    throw new Error(message || `模型资源请求失败（${response.status}）：${url}`);
}
