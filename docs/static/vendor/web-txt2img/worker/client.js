...
export class Txt2ImgWorkerClient {
    ...
    generate(params, onProgress, opts) {
        ...
        const abort = () => {
            // The host acknowledges abort with an "accepted" event instead of a
            // terminal result. Do not add this fire-and-forget control message to
            // `pending`, or each cancellation would retain an unresolved promise.
            this.worker.postMessage({ id: uid(), kind: 'abort' });
        };
