(function () {
    ...
    async function loadLandscapeData(force = false) {
        ...
        try {
            const traceResult = await fetchJsonCandidates(["/static/assets/data/cnn/training_pca_trace.json"]);
            ...
            let surfacePath = "";
            try {
                const surfaceResult = await fetchJsonCandidates(["/static/assets/data/cnn/fc2_loss_surface.json"]);
            ...
            try {
                const lrResult = await fetchJsonCandidates([
                    "/static/assets/data/cnn/lr_comparison_trace.json"
                ]);
